	...
	async analyzeImages(files: Blob[]): Promise<any> {
		...
		results.forEach((res) => {
			Object.entries(res).forEach(([key, val]) => {
				aggregatedCounts[key] = (aggregatedCounts[key] || 0) + val;
			});
		});
		...
		const items = Object.entries(aggregatedCounts).map(([key, count]) => {
			...
			return {
				...
				type: mapping.type,
				count: count,
				estimatedWeight: +totalWeight.toFixed(2),
			};
		});
		...
		return {
			items,
			totalEstimatedWeight: +totalWeight.toFixed(2),
			estimatedCarbonSaved: +estimatedCarbonSaved.toFixed(2),
		};
	}
	...
	async calculateCarbonFootprint(items: any[]): Promise<number> {
		...
		for (const item of items) {
			...
			try {
				...
				else if (type.includes("metal")) factor = 9;
