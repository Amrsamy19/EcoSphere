"use client";
import { Recycle, CheckCircle, Clock, Truck, Eye, Loader2 } from "lucide-react";
import { useState, useEffect, useCallback } from "react";
import {
	fetchRecycleRequests,
	updateRecycleRequestStatus,
	RecycleResponse,
} from "@/frontend/api/Recycle";

// UI status values
type orderStatus = "reviewing" | "pending" | "completed" | "out for delivery";

// Map backend status to UI status
const backendToUIStatus = (backendStatus: string | undefined): orderStatus => {
...
const uiToBackendStatus = (uiStatus: orderStatus): string => {
	switch (uiStatus) {
		case "reviewing":
			return "review";
...
const mapResponseToOrder = (item: RecycleResponse): recycleOrder => {
	const address = item.address;
	const location = `${address.street} ${address.buildingNumber}, ${address.city}`;
	return {
		id: item.id,
...
const RecycleDashboard = () => {
	...
	const loadRequests = useCallback(async () => {
		try {
			setIsLoading(true);
			setError(null);
			const response = await fetchRecycleRequests({ limit: 100 });
			const orders = response.data.map(mapResponseToOrder);
			setRequests(orders);
		} catch (err) {
			console.error("Failed to load recycle requests:", err);
			setError("Failed to load recycle requests. Please try again.");
		} finally {
			setIsLoading(false);
		}
	}, []);
	...
	const handleStatusChange = async (id: string, newStatus: orderStatus) => {
		...
		try {
			...
			await updateRecycleRequestStatus(id, backendStatus);
					...
					{error && !isLoading && (
						<div className="text-center py-12">
							...
							<button
								...
								className="mt-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:opacity-90"
							>
								Retry
							</button>
						</div>
